...
namespace ControleContentoresV2.Controllers
{
    public class telaInicialController : Controller
    {
        ...
        {
            var movimentacao = db.movimentacao.Include(m => m.produtoQuimico).Include(m => m.situacao).Include(m => m.volume).OrderBy(m => m.dt_load).ThenBy(m => m.produtoQuimico.nomeProduto);
            return View(movimentacao.ToList());
        }

        // GET: telaInicial/Details/5
        ...
        public ActionResult RetonarRefil(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
